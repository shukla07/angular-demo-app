...
@Injectable()
export class GetMailByIdAdapter implements IAdapter<EmailLoopModel> {
  ...
  adaptToModel(resp: any): any {
    ...
    loopModel.forwarded = resp.items[0].threadExtMetadata?.forwarded;
    loopModel.forwardedTo = resp.items[0].threadExtMetadata?.to
      .map(user => user.name)
      .join(', ');
    resp.items = resp.items.find(item => item.messageExtMetaData.forwarded)
      ? resp.items
      : resp.items.filter(i => {
    ...
    loopModel.messages = resp.items.map(mail => {
      ...
      model.from = mail.messageExtMetaData ? mail.messageExtMetaData.from : '';
      ...
      model.storage = currentUserGroup
        ? currentUserGroup.storage
        : mail.group[0].storage;
